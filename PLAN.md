...
### 機能要件
- [x] ツリーがディレクトリのみ表示 (Step 1完了)
- [x] ツリーから新しいディレクトリを作成可能 (Step 2完了)
- [x] カテゴリ名がそのままディレクトリ名として使用される (Step 3完了)
- [x] ディレクトリ選択でテンプレート/フロントマター編集可能 (Step 4: データ構造準備完了、UI実装は将来対応)
- [x] Categoryページがツリー表示 (Step 5完了)
- [x] ウィンドウサイズが適切に調整 (既存のサイズで適切に動作)

### 非機能要件
- 既存データが正しくマイグレーションされる (folderフィールド削除対応済み)
